...
class Expdisk(GalfitComponent):
    ...
    def validate(self) -> None:
        ...
        if self.x_pos.value <= 0 or self.y_pos.value <= 0:
            raise ValueError("Expdisk position must be positive (GALFIT coords)")

        if self.rs.value <= 0:
            raise ValueError("Scale length rs must be positive")
    ...
    def to_galfit(self) -> str:
        ...
        lines.append(" 0) expdisk                # object type")
        ...
        lines.append(
            f" 4) {self.rs.value:.4f}   "
            f"{self._fit_flag(self.rs)}  "
            "# R_s (disk scale-length) [pix]"
        )
