...
class Sersic(GalfitComponent):
    ...
    def validate(self):
        PARAMS = ["x_pos", "y_pos", "mag", "eff_rad", "sersic_index", "axis_ratio", "pos_angle"]
        for name in PARAMS:
            param = getattr(self, name)
            self._validate_param(name, param)

        if self.x_pos.value <= 0 or self.y_pos.value <= 0:
            raise ValueError("Sersic position must be positive (GALFIT pixel coords)")
    ...
    def to_galfit(self) -> str:
        ...
        lines.append(" 0) sersic                 # object type")
        ...
        lines.append(
            f" 4) {self.eff_rad.value:.4f}   "
            f"{self._fit_flag(self.eff_rad)}  "
            "# R_e (half-light radius) [pix]"
        )

        lines.append(
            f" 5) {self.sersic_index.value:.4f}   "
            f"{self._fit_flag(self.sersic_index)}  "
        ...
        lines.append(
            f" Z) {z}                    # output option (0 = resid., 1 = Don't subtract)"
        )
